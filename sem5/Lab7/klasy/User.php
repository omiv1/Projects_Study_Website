...
class User {
    ...
    public function saveDB($db) {
        $sql = "INSERT INTO users (userName, fullName, email, passwd, date, status) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $db->getMysqli()->prepare($sql);

        $stmt->bind_param("sssssi", $this->userName, $this->fullName, $this->email, $this->passwd, $this->date, $this->status);
        $stmt->execute();
        $stmt->close();
    }

    public static function getAllUsersFromDB($db) {
        $sql = "SELECT userName, fullName, email, date, status FROM users";
        $pola = ['userName', 'fullName', 'email', 'date', 'status'];
